...
public sealed record CrossSourceActionInvocation(
    Guid InvocationId,
    string SourceId,
    string TargetId,
    HostActionEntryRequestContext HostActionContext)
{
    public bool IsWellFormed(DateTimeOffset now) =>
        ...
        HostActionContext is not null &&
        HostActionContext.Ingress == HostActionEntryIngress.CrossRegistration &&
        HostActionContext.InvocationId == InvocationId &&
        HostActionContext.Contribution?.IngressBinding.Ingress == HostActionEntryIngress.CrossRegistration &&
        string.Equals(HostActionContext.Contribution.IngressBinding.PrimaryIdentity, SourceId, StringComparison.Ordinal) &&
        string.Equals(HostActionContext.Contribution.IngressBinding.SecondaryIdentity, TargetId, StringComparison.Ordinal) &&
        HostActionContext.IsWellFormed(now);
}

/// <summary>Host-issued authority for one typed registration action entry.</summary>
...
/// <summary>Host-issued context capability for one typed registration action entry.</summary>
...
/// <summary>Host-owned entry for typed registration action calls.</summary>
/// <remarks>
...
/// <summary>Authenticated authority for one external registration action dispatch.</summary>
        ...
        DateTimeOffset now)
    {
        ...
        if (!SameDescriptor(authority.Descriptor, expectedDescriptor) ||
            ...
            !SamePayload(
                ...
                expectedDescriptor.InputTypeIdentity,
                expectedDescriptor.InputSchemaVersion) ||
            !string.Equals(authority.SourceId, authority.Call.SourceId, StringComparison.Ordinal) ||
            !string.Equals(authority.GraphId, authority.Call.GraphId, StringComparison.Ordinal) ||
            !string.Equals(authority.SourceId, hostAuthority.SourceId, StringComparison.Ordinal) ||
...
{
    public ActionFailedException(ExecutionError error)
        : base(error.Message)
    {
        Error = error;
    }
