...
public sealed record SidecarDiscoveryEnvelope(
    ...
    IReadOnlyList<FeatureDescriptor> Features) : ISidecarProtocolMessage
{
...
public static class SidecarDiscoveryValidator
{
        ...
        SidecarHostDescriptorCatalog hostCatalog)
    {
        ...
        if (!discovery.Header.Size.IsWithinLimit ||
            ...
            return Reject(SidecarProtocolErrors.PayloadTooLarge, "Discovery exceeds its host size authority.");

        try
        {
            if (JsonSerializer.SerializeToUtf8Bytes(discovery, discovery.GetType()).Length >
                hostCatalog.PayloadLimits.ProtocolMessageBytes)
                return Reject(SidecarProtocolErrors.PayloadTooLarge, "The measured discovery exceeds the host limit.");
        }
        catch (JsonException)
        {
        ...
        if (duplicateAction is not null)
            return Reject(SidecarProtocolErrors.DuplicateDescriptor, "The discovery contains duplicate registration action definitions.");

        var duplicateEvent = discovery.EventDefinitions
            .GroupBy(item => item.EventKey)
            .FirstOrDefault(group => group.Count() > 1);
        if (duplicateEvent is not null)
            return Reject(SidecarProtocolErrors.DuplicateDescriptor, "The discovery contains duplicate registration event definitions.");
        ...
        if (discovery.ActionDefinitions.Any(item => hostCatalog.ContainsActionKey(item.ActionKey)))
            return Reject(SidecarProtocolErrors.ShadowedHostKey, "A registration action definition shadows a host action key.");

        if (discovery.EventDefinitions.Any(item => hostCatalog.ContainsEventKey(item.EventKey)))
            return Reject(SidecarProtocolErrors.ShadowedHostKey, "A registration event definition shadows a host event key.");
            ...
            foreach (var descriptor in actionMatches)
            {
                ...
                if ((subscription.TargetKind is SidecarHookTargetKind.Category or SidecarHookTargetKind.Wildcard) &&
                    ...
                    (hostCatalog.SensitiveWildcardApproval is null ||
                     !string.Equals(hostCatalog.SensitiveWildcardApproval.SourceId, discovery.SourceId, StringComparison.Ordinal) ||
...
public static class SidecarProtocolStateMachine
{
        ...
        DateTimeOffset now)
    {
        ...
        if (!message.Header.Size.IsWithinLimit ||
            ...
            return Reject(SidecarProtocolErrors.PayloadTooLarge, "The message exceeds its size authority.");
        ...
        try
        {
            ...
            if (measuredBytes > hostPayloadLimit ||
                ...
                return Reject(SidecarProtocolErrors.PayloadTooLarge, "The measured protocol message exceeds the host limit.");
        }
...
public static class SidecarProtocolErrors
{
    ...
    public const string PayloadTooLarge = "registration_payload_too_large";
    public const string RegistrationBusy = "registration_busy";
