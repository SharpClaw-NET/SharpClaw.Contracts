...
/// <summary>
/// Host→registration write adapter exposing a minimal surface for upserting
/// <c>ProviderDB</c> and <c>ModelDB</c> rows. Implemented by the host
/// (<c>HostModelRegistrar</c> in <c>Application.Core.Registrations</c>) and
/// consumed by registrations that need to register provider/model rows while
