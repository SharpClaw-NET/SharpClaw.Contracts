...
/// owning registration has been disabled or uninstalled. Carries the missing
/// provider key so callers can surface an actionable
/// <em>"enable registration X to use this provider"</em> message instead of a
...
public sealed class ProviderUnavailableException : Exception
{
    public ProviderUnavailableException(string providerKey)
        : base($"Provider '{providerKey}' is not available. The owning provider registration is disabled or not installed.")
    {
